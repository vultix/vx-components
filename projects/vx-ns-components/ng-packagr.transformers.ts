...
const hooks: NgPackagerHooks = {
  writePackage: {
    before: async taskContext => {
      ...
      const result = await bundler.Bundle('./projects/vx-ns-components/src/theming.scss');

      await writeFile(join(workspaceRoot, 'dist/vx-ns-components/theming.scss'), result.bundledContent);
    }
  }
};
